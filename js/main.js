...
var renderAdvert = function (object) {
  var advertElement = newAdvert.cloneNode(true);

  //advertElement.querySelector('.popup__avatar').src = object.author.avatar;
  //advertElement.querySelector('.popup__title').textContent = object.offer.title;
  //advertElement.querySelector('.popup__text--address').textContent = object.offer.address;
  //advertElement.querySelector('.popup__text--price').textContent = object.offer.price;
  //advertElement.querySelector('.popup__type').textContent = object.offer.type;
  //advertElement.querySelector('.popup__text--capacity').textContent = object.offer.rooms + ' комнат для ' + object.offer.guests + ' гостей';
  //advertElement.querySelector('.popup__text--time').textContent = object.offer.checkin + + object.offer.checkout;
  //advertElement.querySelector('.popup__features').textContent = object.offer.features;
  //advertElement.querySelector('.popup__description').textContent = object.offer.description;
  //advertElement.querySelector('.popup__photos').src = object.offer.photos;
  advertElement.children[0].src = object.author.avatar;
  advertElement.children[0].alt = object.offer.title;
  advertElement.style.left = object.location.x + 'px';
  advertElement.style.top = object.location.y + 'px';
